...
class SingleCommand extends Command {
  async run() {
      ...
      const catalogJson = fs.readFileSync(path.resolve(__dirname,`./templates.json`));
      const catalog = JSON.parse(catalogJson);

      if(!flags.technology){
        Console.error(`Please specify the template technology for you want to download`);
        let choice = await prompts([{
            type: 'select',
            name: 'technology',
            message: 'Pick a template technology:',
      ...
      if(flags.technology){
        Console.info(`Creating new ${flags.technology} template...`);
...
SingleCommand.aliases = ['download:template'];
SingleCommand.description = 'Download a new template or boilerplate'
SingleCommand.flags = {
